...
float trilinearly_interpolate(const float3 point,
    ...
    if (voxel.x < 0 || voxel.y < 0 || voxel.z < 0 || voxel.x >= voxel_grid_size.x || voxel.y >= voxel_grid_size.y ||
        voxel.z >= voxel_grid_size.z) {
...
bool can_intersect_in_dimension(float space_min, float space_max, float origin, float direction, float &near_t,
                                float &far_t) {
...
__device__
bool
compute_near_and_far_t(const float3 &origin, const float3 &direction, const float3 &space_min, const float3 &space_max,
                       float &near_t, float &far_t) {
    bool intersects = false;

    // Handle start_point in space
    if (origin.x >= space_min.x && origin.x <= space_max.x && origin.y >= space_min.y && origin.y <= space_max.y &&
        origin.z >= space_min.z && origin.z <= space_max.z) {
...
void compute_normals(uint16_t width, uint16_t height, const float3 *vertices, float3 *normals) {
    ...
    if (imy == height - 1) {
        ...
        if (imx == width - 1) {
            ...
            float3 v2{vertices[idx + 1].x - vertices[idx].x, vertices[idx + 1].y - vertices[idx].y,
                      vertices[idx + 1].z - vertices[idx].z};
            float3 v1{vertices[idx + width].x - vertices[idx].x, vertices[idx + width].y - vertices[idx].y,
                      vertices[idx + width].z - vertices[idx].z};
...
//                depth_data[i] = (uint16_t)roundf(cam_point.z());
                depth_data[i] = (uint16_t) roundf(
                        world_point.z * 255.99);  // max depth is 1 -> multiply by 255.99 for visualization
            }
