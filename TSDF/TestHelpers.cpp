...
void create_sphere_in_TSDF( TSDFVolumeCPU & volume, float radius ) {
    ...
    // Get size of space
    TSDFVolumeCPU::UInt3 size = volume.size();
    ...
    if ( local_dist ) {
        ...
        TSDFVolumeCPU::Float3 volume_centre = (volume.offset() + volume.physical_size()) / 2.0f;
        TSDFVolumeCPU::Float3 voxel_size = volume.voxel_size();
        ...
        for ( int k = 0; k < size.z; k++ ) {
            for ( int j = 0; j < size.y; j++ ) {
                for ( int i = 0; i < size.x; i++ ) {
                    ...
                    TSDFVolumeCPU::Float3 voxel_centre = volume.offset() + (voxel_size * TSDFVolumeCPU::Float3{i + 0.5f, j + 0.5f, k + 0.5f} );

                    // Compute vector to centre of space
                    TSDFVolumeCPU::Float3 vector = volume_centre - voxel_centre;
